...
def _update_pi():
    ...
    #     stick.set_single_LED_color(i, rgb[i][0], rgb[i][1], rgb[i][2])
    # _prev_pixels = np.copy(p)
    # # strip.show()
    ...
    g = np.left_shift(p[1][:].astype(int), 16)
    b = p[2][:].astype(int)
    # print(f'pixels: {pixels}, p: {p}, r: {r}, g: {g}, b: {b}')
    ...
    for i in range(config.N_PIXELS):
        ...
        # print(f'rgb: {rgb}')
        print(f'pixels: {pixels}')
        stick.set_single_LED_color(i, pixels[0, i], pixels[1, i], pixels[2, i])
    # _prev_pixels = np.copy(p)
    # strip.show()
